...
public:
  ...
  //Postcondition: Returns true if the list is full, otherwise it returns false.
  ...
  //Function to return the maximum number of elements that can be stored in the list.
  ...
  //Function to insert item in the list at the location specified by position.
  //Postcondition: list[position] item; length++;
  //               If position is out of range, the program is aborted.

  listType(int listSize = 50);
  //Constructor
  //Creates an array of the size specified by the parameter listSize; the default aray size is 50.
  //Postcondition: list contains the base address of the array; length = 0; maxsize = listSize;
